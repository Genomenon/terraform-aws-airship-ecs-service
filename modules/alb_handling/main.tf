...
resource "aws_lb_listener_rule" "host_based_routing_custom_listen_host" {
  count = var.create && var.load_balancing_type == "application" && false == var.redirect_http_to_https ? length(var.custom_listen_hosts) : 0
  ...
  condition {
    host_header {
      values = var.custom_listen_hosts
    }
  }
}
...
resource "aws_lb_listener_rule" "host_based_routing_custom_listen_host_redirect_to_https" {
  count = var.create && var.load_balancing_type == "application" && false == var.cognito_auth_enabled && var.redirect_http_to_https && length(var.custom_listen_hosts) > 0 ? 1 : 0
...
resource "aws_lb_listener_rule" "host_based_routing_ssl_custom_listen_host" {
  count = var.create && var.https_enabled && var.load_balancing_type == "application" && false == var.cognito_auth_enabled && length(var.custom_listen_hosts) > 0 ? 1 : 0
...
resource "aws_lb_listener_rule" "host_based_routing_ssl_custom_listen_host_cognito_auth" {
  count        = var.create && var.https_enabled && var.load_balancing_type == "application" && var.cognito_auth_enabled && length(var.custom_listen_hosts) > 0 ? 1 : 0
  listener_arn = var.lb_listener_arn_https
